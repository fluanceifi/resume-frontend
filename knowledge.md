...
- 문제: 사용자가 결제 버튼을 연속으로 누르거나(따닥) 네트워크 재전송·MQ 재처리가 발생하면 동일한 결제가 여러 번 생성/실행될 수 있었습니다. 결제 중복은 돈이 두 번 움직이는 정합성 사고입니다. 처음에는 결제 "실행"에만 분산락을 걸었는데, 부하 테스트 중 동시 요청이 오면 결제 식별자 자체가 여러 번 새로 생성되어, 실행을 막아도 중복 "생성"으로 데이터 일관성이 깨졌습니다.
- 원인 분석: 경쟁 상태는 결제 "실행"이 아니라 "식별자 발급/확인" 단계에서 이미 터졌습니다. 즉 중복 실행뿐 아니라 중복 생성도 막아야 멱등성이 온전히 보장됩니다.
- 해결:
  - 멱등키(Global Transaction UUID)를 백엔드가 결제 의도 생성 시점(`/intent`)에 단 한 번 발급하고, 클라이언트는 그 UUID를 결제 실행 요청에 그대로 되돌려보내게 했습니다. 버튼 클릭과 키 발급을 분리한 것이 핵심입니다.
  - Redis `SET NX`(존재하지 않을 때만 저장)의 원자성으로 확인(GET)과 저장(SET)을 단일 명령으로 처리해, 동일 결제 요청은 최초 한 건만 생성되도록 했습니다. 싱글스레드 Redis라도 GET/SET이 분리되면 그 사이에 다른 요청이 끼어들어 서로를 덮어쓰므로, NX로 원자화한 것입니다.
  - Redis에는 단순히 `"1"`이 아니라 상태와 응답 스냅샷(status, transactionId, responseSnapshot)을 저장해, 재시도 시 기존 성공 응답을 그대로 재현(RETURN_SNAPSHOT)하도록 했습니다.
  - 프론트엔드 오작동 방어: 클라이언트가 함께 보낸 실행 payload(merchantId·userId·amount)를 `:`로 직렬화한 뒤 SHA-256으로 환원한 `requestHash`를 저장/비교합니다. 같은 UUID인데 payload가 다르면(프론트가 결제 키를 재사용하거나 재전송 중 금액·가맹점이 변형된 경우) 첫 결제 결과를 조용히 멱등 반환하는 오결제 대신 `409 IDEMPOTENCY_CONFLICT`로 시끄럽게 실패시킵니다. (Stripe idempotency-key conflict와 동일 패턴)
  - 하나의 UUID로 3중 방어선을 구성했습니다: 플랫폼(Redis `SET NX` 원자적 선점) → 은행(`transaction_uuid` unique 제약 + 원장 선조회로 최종 방어) → 블록체인(온체인 이력 조회로 이중 기입 방지). 한 계층이 흔들려도 다음 계층이 같은 UUID로 중복을 걸러냅니다.
- 결과: K6 기반 부하 테스트(VU10)에서 동시에 결제를 요청해도 성공 1건, 중복 요청 409 9건, 중복 결제 0건으로 동일 결제가 한 번만 생성됨을 검증했습니다. (idempotency_burst_2xx=1, idempotency_burst_blocked_409=9, idempotency_executed_once=1, idempotency_violation=0 커스텀 메트릭으로 확인)
- 배운 점: 결제에서 가장 무서운 건 요청이 실패하는 게 아니라, 실제로는 성공했는데 서버가 실패라고 믿고 사용자가 다시 결제하게 만드는 것입니다. 멱등성은 그 문제를 막는 정합성 장치이고, 클라이언트를 신뢰하지 않고 백엔드가 최종 방어합니다.

### 한강페이 트러블슈팅 2 — 분산 시스템 결제 복구와 트랜잭션 상태 머신

- 문제: 플랫폼-은행-블록체인으로 이어지는 분산 구조에서 서버 경계를 넘는 요청은 하나의 트랜잭션으로 묶이지 않습니다. 은행 호출 중 HTTP 5xx나 타임아웃이 나면 요청이 갔는지, 갔다면 성공했는지조차 알 수 없었습니다. 이를 단순히 "실패"로 처리했더니, 실제로는 성공했는데 사용자에게 잘못 안내되는 정합성 문제가 생겼습니다. 정합성을 맞추려 복구 트랜잭션을 추가했더니, 실패한 복구를 계속 재시도하는 무한 루프로 서버가 다운되는 문제도 발생했습니다.
- 원인 분석: 분산 시스템에서 외부 호출 결과는 성공/실패의 이분법이 아니라 성공·실패·알 수 없음(Unknown) 세 가지입니다. 카카오페이·토스페이먼츠 같은 결제사도 Unknown 상태를 1급으로 두고 결제 도메인에서 무조건적인 롤백은 지양합니다. "알 수 없음"을 성급히 실패로 단정하는 것 자체가 정합성을 깨는 원인이었습니다.
- 해결:
  - 토스페이먼츠 결제 상태 흐름(READY→IN_PROGRESS→DONE/ABORTED/EXPIRED)을 참고해 상태 머신을 설계했습니다: `PENDING`(의도 생성, 은행 실행 요청 전) → `PROCESSING`(은행 실행 요청함) → `SUCCESS`(2xx)/`FAILED`(4xx, 잔액부족 등 결정적 실패)/`UNKNOWN`(5xx·타임아웃), 그리고 복구 불가 시 `EXPIRED` 터미널.
  - `PENDING`과 `PROCESSING`을 분리해, PENDING은 만료시켜도 실제 결제가 발생하지 않고, PROCESSING은 함부로 실패 처리하지 않고 결과를 모르면 UNKNOWN으로 두도록 했습니다.
  - 복구 대상은 무한정이 아니라 상태별 시간 기준으로 선별했습니다: PENDING은 사용자 화면 이탈로 보고 `createdAt+10분` 초과 시, PROCESSING은 은행 API 호출 전 서버 다운으로 보고 `createdAt+5분` 초과 시, UNKNOWN은 5xx/타임아웃 즉시 복구 대상입니다.
  - 재시도는 재실행이 아니라 멱등 재시도입니다. 결제 의도마다 발급한 Global Transaction UUID를 그대로 실어 은행에 재요청하므로, 은행은 이 키로 이미 처리된 건이면 기존 결과를 그대로 반환합니다(카카오페이 `tx_key` 기반 멱등 API, "요청이 성공했는지 확인 후 재시도"와 동일). 덕분에 재시도가 이중 결제로 번지지 않습니다.
  - 무한 루프를 막기 위해 애매한 건은 딱 한 번만 재시도하고, 그래도 UNKNOWN이면 상태를 저장한 뒤 스케줄러(UnknownPaymentRecoveryScheduler)에 위임했습니다. 스케줄러는 PENDING·PROCESSING·UNKNOWN을 5분 주기로 모아 지수 백오프로 최대 10회 은행 조회 API(`GET /api/v1/transactions/{transactionUuid}`)로 상태를 재확인합니다. 은행 서버는 txHash가 있어도 바로 SUCCESS를 주지 않고 블록체인 TransactionReceipt로 결과가 확정된 뒤에만 SUCCESS를 반환하도록 했습니다.
  - 10회를 시도해도 확정되지 않으면 "복구 불가"로 판정해 `EXPIRED` 터미널 상태로 닫고 자동 재시도 루프에서 영구 제외했습니다.
- 결과: 스케줄러를 통해 사용자의 화면 이탈, 타임아웃, 서버 다운 같은 애매한 상황을 고아 상태로 남기지 않고 전부 자동 복구했습니다. 실제 스케줄러 로그(`UnknownPaymentRecoveryScheduler : 결제 복구 스케줄러 실행 : 대상=68`)로 동작을 확인했고, 최종 발표에서 "알 수 없음(Unknown) 상태를 다룬 건 이 팀이 처음"이라는 평을 받았습니다.
- 배운 점: 분산 결제에서는 "모른다"는 상태를 1급 시민으로 다뤄야 하고, 재시도는 멱등키로 안전하게 만들되 조건과 횟수를 명확히 제한해야 무한 루프를 끊을 수 있습니다.
