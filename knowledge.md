...
- **해결**:
  ...
  - **프론트엔드 오작동 방어**: 클라이언트가 함께 보낸 실행 payload(merchantId·userId·amount)를 `:`로 직렬화한 뒤 SHA-256으로 환원한 `requestHash`를 저장·비교합니다. 같은 UUID인데 payload가 다르면(프론트가 결제 키를 재사용하거나 재전송 중 금액·가맹점이 변형된 경우) 첫 결제 결과를 조용히 반환하는 오결제 대신 `IDEMPOTENCY_CONFLICT`(409)로 시끄럽게 실패시킵니다. (Stripe idempotency-key conflict와 동일 패턴)
