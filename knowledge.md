...
- **문제**: 사용자가 결제 버튼을 연속으로 누르거나 네트워크 재전송이 발생하면 같은 결제가 여러 번 생성·실행될 수 있었습니다. 결제 중복은 돈이 두 번 움직이는 정합성 사고입니다. 처음엔 흔한 방법부터 하나씩 대봤는데 전부 막지 못했습니다. `synchronized`는 JVM 안에서만 유효해서 **서버를 이중화하면 의미가 없었고**, DB의 UNIQUE 제약은 요청마다 `TransactionUUID`가 서로 달라서 애초에 **제약에 걸리지 않았으며**, DB 비관적 락은 **INSERT 전이라 잠글 행 자체가 아직 없었습니다.**
- **원인**: 두 가지가 비어 있었습니다. ① **막을 '대상'이 없었다** — 결제 버튼을 두 번 누르면 서버가 `TransactionUUID`를 두 개 발급했습니다. 중복을 막는 모든 장치는 "이게 같은 결제인가?"를 판단하는데, 식별자가 매번 새로 생기면 경합 자체가 성립하지 않았습니다. ② **은행에 물어볼 '식별자'가 없었다** — 식별자를 고정해 락이 동작하게 만들어도, 외부 은행 호출에서 타임아웃이나 5xx가 나면 은행이 처리했는지 되물어봐야 하는데, `TransactionUUID`가 요청을 받은 뒤 서버 메모리에서만 만들어져 은행에 보내기 전 **어디에도 기록되지 않아** 조회할 식별자가 없었습니다.
- **해결**:
  - **결제를 2단계로 분리(기록 후 실행)했습니다.** 실행 전에 '결제 시도'를 먼저 만들어, 서버가 `transactionUuid`(Global Transaction UUID)를 **발급해 DB에 저장**하고 클라이언트는 그 값을 실행 요청으로 되돌려 보냅니다. 발급 주체를 클라이언트에서 서버로 옮긴 셈이라, 버튼을 몇 번 눌러도 GTID는 하나로 고정돼 원인 ①이 풀립니다. 그리고 은행에 보내기 전 이미 DB에 이름이 남아 있어, 거래가 유실돼도 GTID로 은행에 조회해 결과를 확정할 수 있어 원인 ②도 풀립니다.
  - **모든 서버가 공유하는 락은 Redis로 잡았습니다.** Lettuce(스핀하며 계속 재시도, 대기 중 Redis 부하 있음)와 Redisson(Pub/Sub 알림, 대기 부하 거의 없지만 의존성 추가)을 비교했는데, Redisson의 장점은 '기다릴 때'입니다. 하지만 결제는 기다리면 안 되고 **이미 실행 중이면 409로 차단**해야 합니다. 대기 자체가 없으니 Redisson이 풀어주는 문제도 안 생겨서, `setIfAbsent`(SET NX) 한 줄로 충분했고 의존성을 추가하지 않았습니다.
  - **확인과 저장을 하나의 명령으로 처리했습니다.** GET(확인)과 SET(저장)이 나뉘면 그 사이에 다른 요청이 끼어듭니다. `setIfAbsent`(SET NX)의 원자성으로 한 번에 처리하고, Redis가 단일 스레드로 순차 실행하므로 끼어들 틈 없이 선점에 성공한 하나만 진행합니다. 실패는 `PAYMENT_ALREADY_PROCESSING`(409, 재시도 없음)입니다.
  - **클라이언트가 조작할 수 없는 멱등키(`requestHash`)를 썼습니다.** 멱등 판정 해시를 클라이언트가 보낸 값이 아니라 **DB에 저장된 결제 시도의 값**(merchantId·userId·amount)으로 계산합니다. payload를 `:`로 직렬화한 뒤 SHA-256 hex로 환원해 저장·비교하는데, 클라이언트가 실행 요청의 금액을 바꿔 보내도 해시 입력이 서버 기록 기준이라 **해시 자체를 조작할 수 없습니다.** 저장된 해시와 다르면(프론트가 결제 키를 재사용하거나 재전송 중 금액·가맹점이 변형된 경우) 첫 결제 결과를 조용히 반환하는 오결제 대신 `IDEMPOTENCY_CONFLICT`(409)로 시끄럽게 실패시킵니다. (Stripe idempotency-key conflict와 동일 패턴) 금액은 `BigDecimal.stripTrailingZeros()`로 정규화해 `1.0`과 `1.00`이 서로 다른 결제로 갈리지 않게 했습니다.
  - Redis에는 `"1"` 같은 플래그가 아니라 **상태와 응답 스냅샷**(status, transactionId, responseSnapshot)을 저장해, 재시도가 오면 기존 성공 응답을 그대로 재현합니다. 레코드 TTL은 7일입니다.
  - 기존 레코드가 있으면 ① requestHash 불일치 → 충돌(CONFLICT) ② 응답 스냅샷 존재(완료) → 스냅샷 반환 ③ FAILED → 재시도 거절 ④ 그 외 → 처리 중 대기, 순서로 판정합니다.
- **결과**: K6 부하 테스트에서 **VU 10**으로 같은 결제를 동시에 몰아넣어도 **성공 1건, 중복 요청 9건(409), 중복 결제 0건**으로 동일 결제가 한 번만 생성되는 걸 검증했습니다. (커스텀 메트릭 `idempotency_burst_2xx`=1, `idempotency_burst_blocked_409`=9, `idempotency_executed_once`=1, `idempotency_retry_snapshot_returned`=1, `idempotency_violation`=0)
...
- **해결**:
  - 토스페이먼츠 결제 상태 흐름(READY→IN_PROGRESS→DONE/ABORTED/EXPIRED)을 참고해, 성공/실패 외에 결제 시도, 처리 중, 알 수 없음, 복구 불가 상태를 추가한 상태 머신을 설계했습니다.
  ...
  - 복구 대상은 무한정이 아니라 상태별 시간 기준으로 선별합니다. PENDING은 사용자 화면 이탈로 보고 `createdAt+10분` 초과 시, PROCESSING은 은행 호출 전 서버 다운으로 보고 `createdAt+5분` 초과 시, UNKNOWN은 5xx/타임아웃 즉시 복구 대상입니다.
  - 재시도는 **재실행이 아니라 멱등 재시도**입니다. 결제 의도마다 발급한 `transactionUuid`를 그대로 실어 은행에 재요청하므로, 은행은 이 키로 이미 처리된 건이면 기존 결과를 그대로 돌려줍니다(카카오페이 `tx_key` 기반 멱등 API의 "요청이 성공했는지 확인 후 재시도"와 같은 원리). 덕분에 재시도가 이중 결제로 번지지 않습니다.
  ...
  - 끝내 확정되지 않으면 `EXPIRED`(복구 불가) 터미널 상태로 닫아 자동 재시도 루프에서 제외하고, 사람이 확인하게 했습니다.
