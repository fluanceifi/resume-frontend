...
| `api/_data/embeddings.json` | 사전 생성된 청크 임베딩(커밋 대상) |
...
`knowledge.md`를 수정한 뒤 임베딩을 다시 생성하고 커밋한다.

```bash
OPENAI_API_KEY=sk-... npm run embeddings
git add knowledge.md api/_data/embeddings.json && git commit -m "docs: 지식베이스 갱신"
```
