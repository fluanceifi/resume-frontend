...
|---|---|---|---:|---|---:|
| 결제 승인 API 필수 파라미터 | 결제 승인 | 결제 승인 | O | 자동결제 승인 | X |
| paymentKey로 결제 상태 조회 | paymentKey로 결제 조회 | paymentKey로 결제 조회 | O | 거래 조회 | X |
| orderId로 결제를 찾는 방법 | orderId로 결제 조회 | orderId로 결제 조회 | O | 결제 취소 | X |
...
| Payment 객체 주요 필드 | Payment 객체 | Payment 객체 | O | Billing 객체 | X |
| authKey로 빌링키 발급 | 인증 정보(authKey)로 빌링키 발급 | 인증 정보(authKey)로 빌링키 발급 | O | 가상계좌 발급 요청 | X |
| 카드 정보로 빌링키 발급 | 카드 정보로 빌링키 발급 | 카드 정보로 빌링키 발급 | O | Billing 객체 | X |
| 빌링키로 자동결제 승인 | 자동결제 승인 | 자동결제 승인 | O | 자동결제 승인 | O |
| 발급된 빌링키 삭제 | 빌링키 삭제 | 빌링키 삭제 | O | 가상계좌 발급 요청 | X |
| Billing 객체 응답 필드 | Billing 객체 | Billing 객체 | O | Billing 객체 | O |
| 거래 내역 조회 API | 거래 조회 | 거래 조회 | O | 거래 조회 | O |
| 정산 데이터 조회 | 정산 조회 | 정산 조회 | O | 거래 조회 | X |
| 수동 정산 요청 방법 | 수동 정산 요청 | 수동 정산 요청 | O | 수동 정산 요청 | O |
| 현금영수증 발급 요청 API | 현금영수증 발급 요청 | 현금영수증 발급 요청 | O | 현금영수증 조회 | X |
| 현금영수증 발급 취소 API | 현금영수증 발급 취소 요청 | 현금영수증 발급 취소 요청 | O | 현금영수증 조회 | X |
| 현금영수증 조회 API | 현금영수증 조회 | 현금영수증 조회 | O | 현금영수증 조회 | O |
| 웹훅 이벤트 종류 | 웹훅 이벤트 | 웹훅 이벤트 | O | 현금영수증 조회 | X |
